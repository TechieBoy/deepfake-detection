...
class AudioAttention(nn.Module):
    def __init__(self, num_classes, num_channels, base_size=64, dropout=0.2, ratio=16, kernel_size=7):
        super().__init__()

        self.conv = nn.Sequential(
            ConvBlock(in_channels=num_channels, out_channels=base_size),
        ...
        self.attention = ConvolutionalBlockAttentionModule(
            base_size * 8, ratio=ratio, kernel_size=kernel_size
        )
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
...
def get_model(num_classes, num_channels):
    return AudioAttention(num_classes, num_channels)
