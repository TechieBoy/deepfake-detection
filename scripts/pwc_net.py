...
def convert_to_optical_flow(imgOne, imgTwo, outputlocflo, outputlocpng):
    tensorFirst = torch.FloatTensor(numpy.array(PIL.Image.open(imgOne))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
    tensorSecond = torch.FloatTensor(numpy.array(PIL.Image.open(imgTwo))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
    
    tensorOutput = estimate(tensorFirst, tensorSecond)

    with open(outputlocflo, 'wb') as objectOutput:
        numpy.array([ 80, 73, 69, 72 ], numpy.uint8).tofile(objectOutput)
        numpy.array([ tensorOutput.size(2), tensorOutput.size(1) ], numpy.int32).tofile(objectOutput)
        numpy.array(tensorOutput.numpy().transpose(1, 2, 0), numpy.float32).tofile(objectOutput)
    
    #img = flo.convert_from_file(outputlocflo)
    #Image.fromarray(img).save(outputlocpng)
