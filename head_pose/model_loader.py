...
def get_68_3d_model():
    return resource_filename(__name__, "model_3D_68.txt")
...
def _nose_chin_2eyes_2mouth(landmarks):
    ...
    return np.array(
        [
            _centroid_nose(landmarks),
            _centroid_chin(landmarks),
            _centroid_right_eye(landmarks),
            _centroid_left_eye(landmarks),
            _centroid_mouth_right_corner(landmarks),
            _centroid_mouth_left_corner(landmarks),
        ],
        dtype=np.double,
    )
...
def _nose_2eyes_2ears(landmarks):
    return np.array(
        [
            ...
            _centroid_right_eye(landmarks),
            _centroid_left_eye(landmarks),
            _centroid_right_ear(landmarks),
            _centroid_left_ear(landmarks),
        ],
        dtype=np.double,
    )
...
def _whole_face(landmarks):
    a = []
    for i in range(1, 37):
        a.append(landmarks[i])
    a.extend([landmarks[49], landmarks[55]])
    return np.array(a, dtype=np.double)


def _central_face(landmarks):
    a = []
...
def _nose_2eyes(landmarks):
    ...
    """
    return np.array([_centroid_nose(landmarks), _centroid_right_eye(landmarks), _centroid_left_eye(landmarks)], dtype=np.double)
...
def get_model_3d_points(mode="nose_chin_eyes_mouth"):
    ...
    elif mode == "whole_face":
        return _whole_face(model_points)
    elif mode == "central_face":
        return _central_face(model_points)
    else:
        raise ValueError('mode must be either "nose_eyes_ears" or "nose_chin_eyes_mouth" or "nose_eyes_mouth" or "nose_2eyes"')
...
def get_points_from_landmarks(landmarks, mode="nose_chin_eyes_mouth"):
    ...
    if mode == "nose_chin_eyes_mouth":
        ...
        return _whole_face(landmarks)
    elif mode == "central_face":
        return _central_face(landmarks)
    else:
        raise ValueError('mode must be either "nose_eyes_ears" or "nose_chin_eyes_mouth" or "nose_eyes_mouth" or "nose_2eyes"')
