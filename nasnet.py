...
class NASNet(nn.Module):
    def __init__(
            ...
            OrderedDict(
                [
                    (
                        "conv",
                        nn.Conv2d(
                            3, num_stem_features, kernel_size=3, stride=2, bias=False
                        ),
                    ),
                    (
                        "bn",
                        nn.BatchNorm2d(
                            num_stem_features, eps=0.001, momentum=0.1, affine=True
                        ),
                    ),
                ]
            )
        ...
        self.cell_stem_0 = CellStem0(
            in_channels=num_stem_features, out_channels=int(filters * scaling ** (-2))
        )
        self.cell_stem_1 = CellStem1(
            in_channels_x=int(4 * filters * scaling ** (-2)),
        ...
        for i in range(3):
            self.add_module(
                "cell_{:d}".format(cell_id),
                FirstCell(
                    in_channels_left=h_channels,
                    out_channels_left=branch_out_channels // 2,
                    in_channels_right=x_channels,
            ...
            for _ in range(num_normal_cells - 1):
                self.add_module(
                    ...
                    NormalCell(
                        in_channels_left=h_channels,
                        out_channels_left=branch_out_channels,
                        in_channels_right=x_channels,
                        out_channels_right=branch_out_channels,
                    ),
                )
            ...
            if i == 1 and self.use_aux:
                self.aux_features = nn.Sequential(
                    ...
                    nn.BatchNorm2d(
                        num_features=128, eps=0.001, momentum=0.1, affine=True
                    ),
                    nn.ReLU(),
                    nn.Conv2d(
                        in_channels=128,
                        out_channels=768,
                        kernel_size=((14 + 2) // 3, (14 + 2) // 3),
                        bias=False,
                    ),
                    nn.BatchNorm2d(
                        num_features=768, eps=1e-3, momentum=0.1, affine=True
                    ),
                    nn.ReLU(),
                )
        ...
        self.linear = nn.Linear(
            x_channels, self.num_classes
        )  # large: 4032; mobile: 1056

        self.num_params = sum([param.numel() for param in self.parameters()])
        if self.use_aux:
            self.num_params -= sum(
                [param.numel() for param in self.aux_features.parameters()]
            )
            self.num_params -= sum(
                [param.numel() for param in self.aux_linear.parameters()]
            )
...
def NASNetAMobile(num_classes=1000):
    return NASNet(
        32, 4, 44, 2, skip_reduction=False, use_aux=True, num_classes=num_classes
    )


def NASNetALarge(num_classes=1000):
    return NASNet(
        96, 6, 168, 2, skip_reduction=True, use_aux=True, num_classes=num_classes
    )
